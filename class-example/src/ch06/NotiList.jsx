...
// 리스트에서 요소를 렌더링 시, 해당 키 값을 설정을 안하면 오류가 뜸.
// 그리고 리스트에서 키를 설정 시, 속도가 조금 더 빨라짐
const reserveNoti = [
    {id:1, message:"메시지1"},
    {id:2, message:"메시지2"},
    {id:3, message:"메시지3"},
...
// construct, super 사용함. 이유는. props 및 state 사용하기 위해
class NotiList extends Component {
    ...
    // componentDidMount 확인. 마운트가 되고(화면에 그려지고)후, 호출되는 메서드 
    componentDidMount(){
        //  비구조화 할당으로 -> this.state.noti 표현을 하면
        const{noti} = this.state;
        // setInterval 매개변수가 2개있음. 1) 콜백함수 2) 특정시간(ms)
        // 1000ms => 1초
        // 특정시간 후에, 콜백 함수를 호출하는 역할
        // 화살표 함수로 정의가 되었음
        timer = setInterval(() => {
            // noti라는 빈 배열이 기존의 예약된 배열의 길이보다 작으면
            // 무조건 실행
            if(noti.length<reserveNoti.length){
                // 현재  index의 noti의 빈 배열의 길이 : 0
                ...
                // noti는 배열에 요소가 추가되었음.
                this.setState({ // p,s,부,강 -> 업데이트가 발생 -> 다시 그려진다. -> 부모 -> 자식 새로 그린다. 
                        // 빈 배열에, 요소가 하나가 추가된 배열이 재할당
                        noti: noti
                    });
            } else{
                // noti라는 빈배열에 요소를 3개 까지 추가 가능, 그 3개가 초과되면
                // 이 함수가 호출
                // timer : setInterval 함수
                // clearInterval(timer);

                // componentWillUnmount() 콘솔을 띄우기 위해서, 해당 데이터를 비워주는 작업.
                // 데이터가 비워지면, 해당 컴포넌트가 그릴게 없어서 사라진ㄷ.
                // 소멸하기 전에 위에 메소드가 호출이 됨. componentWillUnmount()
                this.setState({
                    // 처음에 noti 빈 배열 -> 요소를 3개 추가하고 -> 3개 초과면, 배열 비우기 -> 빈 배열 할당
                    noti:[]
                })
            }
        }, 1000);
    ...
    render() {
        return (
            ...
            예를 들어,
            noti : 빈 배열 -> 배열에 요소를 추가 push, 객체를 요소(message)추가.
            noti = {message:"메시지1"},{message:"메시지2"},{message:"메시지3"},*/}

            {/* 부모에서 자식으로 props 설정, 여기에서 해당 요소에 키 값을 설정함 
            현재, props로 요소로 message 하나만 있는데 추가로 키도 같이 설정. id도 같이 설정
            id를 키 요소의 값으로 사용할 예정 */}
